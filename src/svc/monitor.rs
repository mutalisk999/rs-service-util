use chrono::prelude::*;
use etcd_rs::{Client, ClientConfig, EventType, KeyRange, RangeRequest};
use log::{debug, warn};
...
impl MonSvcClient {
    ...
    ) -> Result<MonSvcClient, Box<dyn Error>> {
    ...
    pub async fn monitor_service(
        self: &mut Self,
        key_prefix: String,
        put_callback: &'static (dyn Fn(String, String) -> () + Sync),
        delete_callback: &'static (dyn Fn(String) -> () + Sync),
        ...
        for kv in resp_range.take_kvs() {
            self.svc_map
                .lock()
                .await
                .insert(kv.key_str().to_owned(), kv.value_str().to_owned());
...

        {
            ...
            tokio::spawn(async move {
                let mut inbound = client
                    .lock()
                    .await
                    .watch(KeyRange::prefix(key_prefix.clone()))
                    .await
                    .unwrap();
                loop {
                            ...
                            Ok(resp) => {
                                if resp.is_some() {
                                    for mut ev in resp.unwrap().take_events() {
                                        ...
                                        match ev.event_type() {
                                            EventType::Put => {
                                                svc_map.lock().await.insert(
                                                    kv.key_str().to_owned(),
                                                    kv.value_str().to_owned(),
                                                );
                                                put_callback(
                                                    kv.key_str().to_owned(),
                                                    kv.value_str().to_owned(),
                                            ...
                                            EventType::Delete => {
                                                svc_map
                                                    .lock()
                                                    .await
                                                    .remove(&kv.key_str().to_owned());
                                                delete_callback(kv.key_str().to_owned());
                                                debug!(
                                                    "service watcher delete {:?} at {:?}",
                                                    kv.key_str(),
                                                    Utc::now().to_string()
                                                );
                                            }
